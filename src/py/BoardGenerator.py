...
class BoardGenerator:
    ...
    def generate(random: Random) -> Board:
        ...
        wantedEmptyCells: int = (
            random.nextInt(Config.MAX_EMPTY_CELLS + 1) if Game.ENABLE_HOLES else 0
        )
        actualEmptyCells: int = 0
        while actualEmptyCells < wantedEmptyCells - 1:
            randIndex: int = random.nextInt(coordListSize)
            randCoord: CubeCoord = coordList[randIndex]
            if BoardGenerator.board[randCoord].getRichness() != Constants.RICHNESS_NULL:
                BoardGenerator.board[randCoord].setRichness(Constants.RICHNESS_NULL)
                actualEmptyCells += 1
